...
import { bookingSchema } from "../../types/validation-booking";
import useCreateBookingTransaction from "../../hooks/booking";
...
export default function BookingForm() {
  ...
  const [totalAmountWithUniqueCode, setTotalAmountWithUniqueCode] = useState<
    number | undefined
  >(0);
  const { mutate, isSuccess, isPending, data } = useCreateBookingTransaction();
  ...
  const {
    ...
    defaultValues: {
      ...
      phone_number: "",
      started_at: "",
      office_space_id: office?.data.id,
    },
  });
  ...
  useEffect(() => {
    ...
    const grandTotal =
      office?.data.price && office.data.price - generatedUniqueCode;

    setUniqueCode(generatedUniqueCode);
    setTotalAmountWithUniqueCode(grandTotal);
  }, [office?.data.price, slug]);
      ...
      <div className="flex flex-col shrink-0 w-[500px] h-fit rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center rounded-full border border-[#000929] px-5 gap-[10px] transition-all duration-300 focus-within:ring-2 focus-within:ring-[#207ECA] overflow-hidden">
              <img
            ...
            {errors.started_at && (
              <span className="text-red-500">Started at is required</span>
            )}
          </div>
        </div>
        <hr className="border-[#F6F5FD]" />
      ...
      <div className="flex flex-col shrink-0 w-[400px] h-fit rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
        ...
        <div className="flex flex-col gap-5">
          <div className="flex items-center justify-between">
            ...
            <p className="font-bold">{office?.data.duration} Days Working</p>
          </div>
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col gap-[2px]">
              <div className="flex items-center gap-1">
                <p className="font-semibold">Locafice</p>
                <img
        ...
        <button
          ...
          <span>{isPending ? "Loading..." : "I’ve Made The Payment"}</span>
        </button>
      </div>
    </form>
  );
